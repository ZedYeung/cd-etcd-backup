...
# TODO VPN CONNECT
echo "Restore..."
LATEST_FULL_ENC_BACKUP=$(s3cmd -c ${FULL_BACKUP_S3CFG} ls -tp ${FULL_BACKUP_OBJECT_STORAGE_BUCKET} | head -n 1)
LATEST_DIFF_ENC_BACKUP=$(s3cmd -c ${DIFF_BACKUP_S3CFG} ls -tp ${DIFF_BACKUP_OBJECT_STORAGE_BUCKET} | head -n 1)
LATEST_FULL_BACKUP=$(${LATEST_FULL_ENC_BACKUP} | rev | cut -f 2- -d '.' | rev)
LATEST_DIFF_BACKUP=$(${LATEST_DIFF_ENC_BACKUP} | rev | cut -f 2- -d '.' | rev)

s3cmd -c ${FULL_BACKUP_S3CFG} get ${FULL_BACKUP_OBJECT_STORAGE_BUCKET}/${LATEST_FULL_ENC_BACKUP} ${LATEST_FULL_ENC_BACKUP}
s3cmd -c ${DIFF_BACKUP_S3CFG} get ${DIFF_BACKUP_OBJECT_STORAGE_BUCKET}/${LATEST_DIFF_ENC_BACKUP} ${LATEST_DIFF_ENC_BACKUP}
